...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformLevelVelocityCommandCfg(
        ...
        limit_ranges=mdp.UniformLevelVelocityCommandCfg.Ranges(
            lin_vel_x=(-1.0, 1.5), lin_vel_y=(-0.3, 0.3), ang_vel_z=(-0.8, 0.8)
        ),
    )
...
class ActionsCfg:
    ...
    JointPositionAction = mdp.JointPositionActionCfg(
        asset_name="robot", joint_names=[".*"], scale=0.25, use_default_offset=True, clip={".*": (-12.0, 12.0)}
    )
...
class RewardsCfg:
    ...
    action_rate_l2 = RewTerm(func=mdp.action_rate, weight=-0.008)
    undesired_contacts = RewTerm(
        func=mdp.undesired_contacts,
        weight=-1.0,
    ...
    feet_air_time = RewTerm(
        ...
        weight=0.15,
        params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*ankle_roll.*"), "threshold": 0.4},
    )
    feet_slide = RewTerm(
        func=mdp.feet_slide,
        weight=-0.25,
...
@configclass
class RobotPlayRoughEnvCfg(RobotPlayEnvCfg):
    def __post_init__(self):
        super().__post_init__()
        self.scene.terrain = TerrainImporterCfg(
            prim_path="/World/ground",
